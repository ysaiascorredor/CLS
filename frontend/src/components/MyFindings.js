...
function MyFindings({ language }) {
  ...
  return (
    <div className="p-6">
      ...
      {selectedFinding && showMessages && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg w-full max-w-2xl max-h-[80vh] flex flex-col">
            <div className="p-6 border-b">
              <h3 className="text-xl font-bold">{t.messages}</h3>
              <p className="text-sm text-gray-600 mt-1">{selectedFinding.question}</p>
            </div>
                ...
                messages.map((msg, idx) => (
                  <div key={idx} className="bg-white rounded-lg p-4 shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      <span className="font-semibold text-blue-600">{msg.user_name}</span>
            ...
            <div className="p-4 border-t bg-white">
              ...
              <button
                onClick={() => {setShowMessages(false); setSelectedFinding(null);}}
                className="w-full mt-3 px-4 py-2 border rounded hover:bg-gray-100"
              >
                {t.cancel}
              </button>
            </div>
